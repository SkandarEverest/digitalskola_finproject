...
class Transformer():
    ...
    def create_province_daily(self):
        ...
        sql = "SELECT * FROM dim_case"
        df_case_dim = pd.read_sql(sql,con=self.engine_postgres)
        # print(df_case_dim)
        ...
        data = data.melt(id_vars=['tanggal','kode_prov'],var_name='status',value_name='total')
    ...
    def create_district_daily(self):
        ...
        data = df[column_start]
        data = data.melt(id_vars=['tanggal','kode_kab'],var_name='status',value_name='total')
